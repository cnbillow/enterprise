...
//	admin	111111
    ...
    public UserAction() {
//		Assign values ​​to the properties of the parent class
...
// When logging in, this field is reserved in the session to record the login status.
        if (session.getAttribute("manage_session_user_info") != null) {
// The status of the login, directly to the management page
...
// Facilitate all data for cookies, see if there are any cookies named manage_cookie_username and manage-cookie-password
// If yes, assign it to username and password
//				session is saved for twenty minutes by default
                sCookie = cookies[i];
                if (sCookie != null) {
...
//		If there is a cookie, it will be automatically logged in.
...
//
    ...
    public String login(HttpSession session, HttpServletResponse response, @ModelAttribute("e") User e, ModelMap model)
            throws Exception {
        String remember = RequestHolder.getRequest().getParameter("remember");
        String errorMsg;
// If the user data of the session is still valid, go directly to the background page.
        if (session.getAttribute("manage_session_user_info") != null) {
            return "redirect:/manage/user/home";
        }
// Verify, determine if it is empty or the verification code is wrong
...
//		use the query basedao to query a piece of data
//		password md5 transcoding
...
// Determine whether the automatic login is checked, and the setting of the cookie is implemented.
        ...
        Collection<MenuItem> userMenus = loadMenus();
//        the background menu is placed in the session
        session.setAttribute("userMenus", userMenus);
        try {
            insertLog(u, "login");
        } catch (Exception ex) {
...
//	}
            if (item.isRootMenu()) {
//                the values ​​here are all corresponding to pid 0
//                note here is the getid
...
//            pid is not zero and the type is not button
            if (!item.isRootMenu() && !item.isButton()) {
// If the id and the root menu have the same id, the contents of the menu
// are placed in the corresponding location in the root.
                MenuItem parentItem = root.get(item.getPid());
                if (parentItem != null) {
//                    focus on methods in an arraylist
                    parentItem.addChildren(item);
                }
            }
        }
//        returns the value of the entire root menu
        return root.values();
    }
    ...
    public String logout(@ModelAttribute("e") User e, HttpServletRequest request, HttpServletResponse response)
            throws Exception {
// Get the session, add the log logout of the user, clear the data of the manage_session_user_info in the session
    ...
    private void insertLog(User u, String log) {
        ...
        String address = null;
//        determine if it is a local test
        if (!systemlog.getLoginIp().equals("127.0.0.1") && !systemlog.getLoginIp().equals("localhost")) {
            ...
            // 异地登陆的判断方法为：先比较本次登陆和上次登陆的区域位置，如果不一致，说明是异地登陆；
            // 如果获取不到区域，则比较IP地址，如果IP地址和上次的不一致，则是异地登陆
//            get the most recent log based on username
    ...
    public String update(HttpServletRequest request, @ModelAttribute("e") User user, RedirectAttributes flushAttrs)
            throws Exception {
    ...
    private void imgCut(InputStream is, String srcImageFile, String savePath, int x, int y, int desWidth,
        ...
        try {
            ...
            if (srcWidth >= desWidth && srcHeight >= desHeight) {
                ...
                bfImage.getGraphics().drawImage(img.getScaledInstance
    ...
    @ResponseBody
// This project does not provide a registration page. Here, specify the add user to determine whether the account and administrator name already exist. If it exists, return error information.
