...
<script>
        ...
        if($("#title").val()!='' && $("#name").val()!='' && $("#tel").val()!='' && $("#email").val()!='' && $("#content").val()!='') {
            $.ajax({
                ...
                success: function (data) {
                    // After the request is successful, the data is emptied
                    $('#messageForm')[0].reset();
                    //After the request is successful, some effects are displayed
                    if(data=="ok"){
     ...
     */
    function reloadImg() {
        //This request address is to return a random verification code.
