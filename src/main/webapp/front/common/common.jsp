...
<head>
    ...
    <!-- 2019-07-25 添加网站统计代码 -->
    <%=SystemManage.getInstance().getSystemSetting().getStatisticsCode()%>
    <script type="text/javascript">
    
    	document.title="<%=SystemManage.getInstance().getSystemSetting().getTitle()%>";
	    function setTitle(a){
	    	debugger;
	    	if(a == "" || a == null || a == undefined){ // "",null,undefined
	    		document.title="<%=SystemManage.getInstance().getSystemSetting().getTitle()%>";
	        }else{
				document.title=a+"_<%=SystemManage.getInstance().getSystemSetting().getTitle()%>";
	        }
		}
    </script>
