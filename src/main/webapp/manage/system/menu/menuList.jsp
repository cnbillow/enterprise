...
var setting = {
		check: {
			...
			onMouseDown: function (event, treeId, treeNode) {
				// According to the id, a piece of data is pointed to the addOrUpdate page.
	...
	$.ajax({
		url:"<%=path%>/manage/menu/getMenusByPid?pid=0",
					...
					success:function(data, textStatus){
						...
						// Request success, after getting the data, hide the loaded animation, display the menu tree
						$("#loadImg").hide();
						$("#treeDemo2").show();
					},
					error:function(){
...
// 					alert(nodes[i].id);
				...
				$.ajax({
					...
					type:"post",
					//ajax passes this property in the past, and the server uses this to determine whether to delete the parent node. Implementation steps in MenuServiceImpl
...
// 						$.fn.zTree.init($("#treeDemo2"), setting, zNodes);
						if(data==1){
							// If the deletion is successful, then reload the menu tree
							loadMenusTree();
						}else{
							alert("删除菜单失败！");
						}
